...
async def process_callback(callback_query: types.CallbackQuery):
    ...
    if button_data == 'my_subscriptions':
        await show_subscriptions(chat_id, callback_query.message.message_id)
...
async def show_subscriptions(chat_id, message_id):
    subscriptions = ['Подписка 1', 'Подписка 2', 'Подписка 3', 'Подписка 4', 'Подписка 5', 'Подписка 6']
    subscriptions_per_row = 2
    markup = subscription_keyboard(subscriptions, subscriptions_per_row)

    await bot.edit_message_text('Ваши подписки:', chat_id, message_id, reply_markup=markup)
...
async def process_subscription(callback_query: types.CallbackQuery):
    ...
    await bot.edit_message_text(f'Выбрана подписка: {subscription_name}', chat_id, callback_query.message.message_id,
                                reply_markup=markup)


async def process_unsubscribe(callback_query: types.CallbackQuery):
    subscription_name = callback_query.data.replace('unsubscribe_', '')
    chat_id = callback_query.message.chat.id

    await bot.edit_message_text('Подписка отменена', chat_id, callback_query.message.message_id)


async def show_anime_list(chat_id, message_id, page=1):
    markup = await anime_keyboard(page=page)
    await bot.edit_message_text('Список аниме:', chat_id, message_id, reply_markup=markup)
...
def register_start(dp: Dispatcher):
    dp.register_message_handler(start, commands=['test'])
